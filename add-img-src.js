...
function fetchMCInumber(cardIndex, printing) {
    ...
    var mciNumber = sets[printing].cards[name].mciNumber;
    if (mciNumber) {
...
for (var i = 0; i < printings.length; i++) { // i == card.name
    for (var k = 0; k < printings[i].length; k++) {
        ...
        if (mci) {
            printings[i][k] = fetchMagicInfoCode(printings[i][k]) + "/" + mci;
            //console.log(printings[i]);
        }
        else {
            // console.log('splicing: ', printings[i][k])
            printings[i].splice(k, 1);
        }
    }
}
for (var i in values) {
